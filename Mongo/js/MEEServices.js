...
const BusinessStoreService_1 = require("./BusinessStoreService");
//定义azure实体
...
let newUserTableSettings = StorageConfig_mongo_1.StorageConfig_mongo.getNewUserTableSettings();
let userTable = new MeeUserTable_1.MultiUserTable(StorageConfig_mongo_1.StorageConfig_mongo.getUserTableSettings(), newUserTableSettings, Config_1.Config.readFromNewUserTable);
// newUserTable: Used for test paths, should switch to this when we remove the old table.
//newUserTable：用于测试路径，在删除旧表时应该切换到这个位置。
let newUserTable = new MeeUserTable_1.UserTable(newUserTableSettings);
let siginTelemtryTable = new MeeSignInTable_1.SignInTelemetryTable(StorageConfig_mongo_1.StorageConfig_mongo.getTelemetryTableSettings());
let receiptsTable = new MeeReceiptsTable_1.ReceiptsTable(StorageConfig_mongo_1.StorageConfig_mongo.getReceiptsTableSettings());
const appInsights = require("applicationinsights");
let instrumentationKey = process.env.APPINSIGHTS_INSTRUMENTATIONKEY ? process.env.APPINSIGHTS_INSTRUMENTATIONKEY : 'a83286f4-84b8-44ea-9f10-b05513259137';
...
//监听警告
function trackWarning(error) {
    appInsightsClient.trackEvent("warning", { content: JSON.stringify(error) });
}
exports.trackWarning = trackWarning;
//监听事件
function trackEvent(name, body) {
    appInsightsClient.trackEvent(name, { error: JSON.stringify(body) });
}
exports.trackEvent = trackEvent;
//监听依赖
function trackDependency(name, command, duration, success) {
...
function signIn(res, user, reqBody) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield Logger_1.logActivity('server-signin', user.tenantId, user.unique_name, reqBody);
            yield signTheUserIn(res, user);
        }
        catch (err) {
            trackException(err);
            yield Logger_1.logActivity('signin-error', user.tenantId, user.unique_name, err);
            res.send(JSON.stringify({ isValid: false }));
        }
    });
}
exports.signIn = signIn;
function signTheUserIn(res, userID) {
    return __awaiter(this, void 0, void 0, function* () {
        let isWhiteListed = (isUserInWhitelistedDomain(userID) || (yield isUserOnIndividualWhitelist(userID)));
        let doLicenseCheck = !isWhiteListed;
        let startTime = Date.now();
        let userEntity = yield userTable.get(userID);
        let duration = Date.now() - startTime;
        trackDependency("azure-storage/meeservicesstorage", "retrieveEntity", duration, true);
        let isExistingUser = userEntity.exists;
        let user = userEntity.entity;
        // TODO: Move logging to blob storage
        yield Logger_1.logActivityVerbose('signin-azurehelper-retrieveentity', userID.tenantId, userID.unique_name, userEntity);
        ...
        if (isExistingUser && (userEntity.entity.isLicensed === undefined)) {
            yield Logger_1.logActivityVerbose('signin-ea-removal', userID.tenantId, userID.unique_name, userEntity);
            yield userTable.delete(userEntity.entity);
            isExistingUser = false;
        }
        let isLicensed = true;
        let licenseType;
        let lastLicenseCheck;
        if (doLicenseCheck && isExistingUser && (user.isLicensed !== undefined)) {
            if (user.lastLicenseCheck !== undefined) {
                ...
                if (currentTime.diff(lastCheck, 'days') < Config_1.Config.licenseCacheDuration) {
        ...
        if (doLicenseCheck) {
            userID.tenancyInfo = yield BusinessStoreService_1.CheckTenancy.CheckAccount(userID);
            if (userID.tenancyInfo.isEduTenant) {
                let licenseCheckResult = yield LicenseManager.EnsureUserIsLicensed(userID, (isExistingUser ? user.anonimizedOid : ""));
                ...
                yield Logger_1.logActivity('signin-not-edu-tenant', 'user not on any white list nor in edu tenancy', userID.unique_name, {});
                res.send(JSON.stringify({ isValid: false }));
                return;
            }
        }
        ...
        if (isExistingUser && (user.lastRoleCheck !== undefined)) {
            ...
            if (currentTime.diff(lastCheck, 'days') < Config_1.Config.roleCheckCacheDuration) {
                role = user.role;
            }
        }
        if (!role) {
            role = yield getUserRole(userID);
            lastRoleCheck = new Date();
        }
        let userInfo;
        if (isExistingUser) {
            userInfo = yield userTable.updateExistingUser(userID, user, role, lastRoleCheck, isLicensed, licenseType, lastLicenseCheck);
        }
        else {
            userInfo = yield userTable.addNewUser(userID, role, isLicensed, licenseType, guid_1.Guid.newGuid());
        }
        let signedToken = createSignedToken(userID);
        let clientSettings = ((userEntity.exists && user.clientSettings) ? user.clientSettings : "");
        let experience = Experience_1.Experience.getExperience(userID, userInfo, isExistingUser);
        let result = makeSignInResult(!isExistingUser, userInfo, getMinecraftName(userID), signedToken, clientSettings, experience);
        yield Logger_1.logActivity('signin-success', userID.tenantId, userID.unique_name, JSON.stringify(result));
        ...
        if (experience && experience.transformExperienceState != null) {
            try {
                ...
                yield userTable.storeExperienceState(userInfo.user);
            }
            catch (error) {
                trackException(error);
                yield Logger_1.logActivity('transformexperiencestate-error', user.tenantId, user.unique_name, error);
            }
        }
        if (Config_1.Config.switchToNewSigninTelemetryTable) {
            yield siginTelemtryTable.recordUser(userInfo.user, !isExistingUser, isWhiteListed, userID);
        }
        else {
            yield recordSigninTelemetry(userID, (!isExistingUser).toString(), isWhiteListed.toString(), isLicensed.toString(), licenseType, role, (1 + userInfo.remainingTrialCount), userInfo.user.skin);
        }
    });
}
function isUserInWhitelistedDomain(user) {
...
function isUserOnIndividualWhitelist(user) {
    return __awaiter(this, void 0, void 0, function* () {
        let allowListEntity = yield AzureHelper.retrieveEntity(AzureHelper.Table.AllowList, user.unique_name, "");
        return allowListEntity.valid;
    });
}
// == edu_earlyaccess version of signin ====================================================================
function signInEarlyAccess(res, user, reqBody) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield Logger_1.logActivity('server-signInEarlyAccess', user.tenantId, user.unique_name, reqBody);
            let userEntity = yield AzureHelper.retrieveEntity(AzureHelper.Table.Users, user.tenantId, user.unique_name);
            let result;
            if (userEntity.valid === true) {
                ...
                result = makeSignInResultEaryAccess(userEntity.azureResult.role["_"], user, userEntity.azureResult.skin["_"], acceptedEula);
            }
            else {
                ...
                let role = yield getUserRoleEarlyAccess(user);
                if (isAllowed) {
                    result = yield createNewUserEarlyAccess(role, user);
                }
                else {
                    let allowListEntity = yield AzureHelper.retrieveEntity(AzureHelper.Table.AllowList, user.unique_name, "");
                    if (allowListEntity.valid === false) {
                        let tenancyInfo = yield BusinessStoreService_1.CheckTenancy.CheckAccount(user);
                        if (tenancyInfo.isEduTenant === false) {
                            yield Logger_1.logActivity('signinEarlyAccess-error', user.tenantId, user.unique_name, "Invalid tenant -- not EDU");
                            res.send(JSON.stringify({ isValid: false }));
                            return;
                        }
                    }
            ...
            trackException(error);
            yield Logger_1.logActivity('signinEarlyAccess-error', user.tenantId, user.unique_name, error);
            res.send(JSON.stringify({ isValid: false }));
        }
    });
}
exports.signInEarlyAccess = signInEarlyAccess;
/**
 ...
 */
function classroomModeSignin(res, userID) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let role = yield getUserRole(userID);
            let returnPayload = {
                "role": role,
            ...
            if (Config_1.Config.switchToNewSigninTelemetryTable) {
                ...
                yield userTable.get(userID).then((user) => {
                    return siginTelemtryTable.recordUser(user.entity, !user.exists, undefined, userID);
                });
            }
            else {
                yield recordSigninTelemetry(userID, '', '', '', '', role, -1, '');
            }
        }
        catch (err) {
            trackException(err);
            res.send(JSON.stringify({ isValid: false }));
            yield Logger_1.logActivity('cmsignin-error', userID.tenantId, userID.unique_name, err);
        }
    });
}
exports.classroomModeSignin = classroomModeSignin;
function registerUserGuid(res, user, ipAddress) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let guid = uuid.v4();
            let role = yield getUserRole(user);
            let newEntry = {
            ...
            yield AzureHelper.insertEntity(AzureHelper.Table.UserGuid, newEntry);
            let returnPayload = {
                "guid": guid
            };
            res.send(JSON.stringify(returnPayload));
            ...
            trackException(err);
            yield Logger_1.logActivity('registerUserGuid-error', user.tenantId, user.unique_name, err);
            res.send(JSON.stringify({ isValid: false }));
        }
    });
}
exports.registerUserGuid = registerUserGuid;
function validateUserGuid(res, guid) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            let guidTableEntity = yield AzureHelper.retrieveEntity(AzureHelper.Table.UserGuid, guid, guid);
            ...
            if (azResult) {
                ...
                yield AzureHelper.updateEntity(AzureHelper.Table.UserGuid, azResult);
            }
            else {
                yield Logger_1.logActivity('validateUserGuid-error', '#error', 'not found', guid);
                res.send(JSON.stringify({ isValid: false }));
            }
        }
        catch (err) {
            trackException(err);
            yield Logger_1.logActivity('validateUserGuid-error', '#error', err.message, guid);
            res.send(JSON.stringify({ isValid: false }));
        }
    });
}
exports.validateUserGuid = validateUserGuid;
function setSkin(res, user, newSkin) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield newUserTable.setSkin(user, newSkin);
        }
        catch (error) {
            trackException(error);
            yield Logger_1.logActivity('setSkin-error', '#error', error.message, newSkin);
        }
        finally {
...
function acceptEula(res, user) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield newUserTable.updateEula(user, true);
        }
        catch (error) {
            trackException(error);
            yield Logger_1.logActivity('updateEula-error', '#error', error.message, undefined);
        }
        finally {
            res.send(JSON.stringify({ isValid: true }));
        }
...
function setReceipt(res, anonimizedOid, user, receiptString) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        appleReceiptVerify.setup()
            ...
            appleReceiptVerify.validateOnce(receiptString, "a858f951a77a49829af3140e2e1e8e40")
                ...
                .catch(function (err) {
                Logger_1.logActivityVerbose('appleReceiptVerify.validate', user.tenantId, user.unique_name, err.message.substring(0, 1024));
                res.send(JSON.stringify({ isValid: false }));
            });
        })
            .catch(function (err) {
            Logger_1.logActivityVerbose('appleReceiptVerify.setup', user.tenantId, user.unique_name, err.message.substring(0, 1024));
            res.send(JSON.stringify({ isValid: false }));
        });
    });
}
exports.setReceipt = setReceipt;
function getUserInfo(res, unique_name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            let entity = yield newUserTable.querybyUniqueName(unique_name);
...
function deleteUser(res, unique_name) {
    return __awaiter(this, void 0, void 0, function* () {
        if (environment_1.Environment.isProduction()) {
            res.sendStatus(403);
            return;
        ...
        try {
            ...
            if (!entity || (entity.length != 1)) {
                ...
                yield newUserTable.delete(entity[0]);
            }
...
function setTrialCounts(res, unique_name, trialsAllowed, trialsUsed) {
    return __awaiter(this, void 0, void 0, function* () {
        if (environment_1.Environment.isProduction()) {
            res.sendStatus(403);
            return;
        ...
        try {
            ...
            if (!entity || (entity.length != 1)) {
                ...
                let userId = AAD_1.UserID.fromUser(entity[0]);
                yield newUserTable.setTrialCounts(userId, trialsAllowed, trialsUsed);
                res.send(JSON.stringify(yield newUserTable.get(userId)));
            }
        }
        catch (error) {
            res.send(JSON.stringify({ "error": error.message }));
...
exports.setTrialCounts = setTrialCounts;
function send404(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield Logger_1.logActivityVerbose('http-error', "404", logline, req.body);
        res.type('text/plain');
        res.status(404);
...
function send500(req, res, err) {
    return __awaiter(this, void 0, void 0, function* () {
        yield Logger_1.logActivityVerbose('http-error', "500", req.url, err);
...
// == HELPER FUNCTIONS ============================================================================
function makeSignInResultEaryAccess(role, user, skinName, acceptedEula) {
...
function makeSignInResult(isNewUser, userInfo, minecraftName, signedToken, clientSettings, popup) {
    let user = userInfo.user;
    let userid = Config_1.Config.returnAnonimizedId ? user.anonimizedOid : user.oid;
    let result = {
        isValid: true,
        isLicensed: user.isLicensed,
        isNewUser: isNewUser,
...
function getMinecraftName(user) {
    const re = /^[_ ]+$/;
...
function createNewUserEarlyAccess(role, user) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let newUser = {
            ...
            "role": entGen.String(role),
            "trialsAllowed": entGen.Int32(Config_1.Config.startingTrialCountStudent),
        ...
        if (role === "teacher") {
            ...
            newUser["trialsAllowed"] = entGen.Int32(Config_1.Config.startingTrialCountTeacher);
        }
        yield AzureHelper.insertEntity(AzureHelper.Table.Users, newUser);
...
function getUserRole(user) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            var success = false;
            let startTime = Date.now();
            var result = yield getUserRoleRisky(user);
            let duration = Date.now() - startTime;
            success = true;
            trackDependency("portal.office.com/getUserRole", "getUserRole", duration, success);
            yield Logger_1.logActivityVerbose('signin-getuserrole', user.tenantId, user.unique_name, "");
            return result;
        }
        catch (err) {
            trackWarning("role check failed: " + err);
            return yield getUserRoleEarlyAccessHack(user);
        }
    });
}
function getUserRoleRisky(user) {
    return __awaiter(this, void 0, void 0, function* () {
        let endpoint = 'https://signup.microsoft.com/';
        let accessToken = yield AAD_2.getOnBehalfOfToken(user, endpoint);
        ...
        return new Promise(function (resolve, reject) {
            try {
                request({
                    ...
                    body: JSON.stringify({ "emailaddress": `${user.unique_name}` })
                    ...
                    try {
                        ...
                        Logger_1.logActivity('role-check-failure', user.tenantId, user.unique_name, body);
                        throw err;
                    }
                });
            }
...
function roleFromPersona(persona) {
    /*
...
function getUserRoleEarlyAccessHack(user) {
    return __awaiter(this, void 0, void 0, function* () {
        let resourceId = 'https://graph.windows.net';
        let accessToken = yield AAD_2.getOnBehalfOfToken(user, resourceId);
        return new Promise(function (resolve, reject) {
            request({
                ...
                let role = "student";
                if (!error) {
                    let bodyText = body.toLocaleLowerCase();
...
function getUserRoleEarlyAccess(user) {
    return __awaiter(this, void 0, void 0, function* () {
        let resourceId = 'https://graph.windows.net';
...
function recordSigninTelemetry(user, isNewUser, isWhiteListed, isLicensed, licenseType, role, remainingTrialCount, skin) {
    return __awaiter(this, void 0, void 0, function* () {
        if (user.osVersion.indexOf('test') >= 0 &&
            (environment_1.Environment.getEnvironmentType() == environment_1.EnvironmentType.Staging) ||
            (environment_1.Environment.getEnvironmentType() == environment_1.EnvironmentType.Production)) {
            return;
        }
        ...
        yield AzureHelper.insertEntity(AzureHelper.Table.SigninTelemetry, signinTelemetryEntity);
    });
}
function getAuthPrivateKey() {
    let key = environment_1.Environment.isLocalDev() ? String(fs.readFileSync('private.pem')) : process.env.PRIVATE_PEM;
    return Utilities.replacePipesWithLineFeeds(key);
}
function getAuthPublicKey() {
    let key = environment_1.Environment.isLocalDev() ? String(fs.readFileSync('public.pem')) : process.env.PUBLIC_PEM;
    return Utilities.replacePipesWithLineFeeds(key);
}
...
//# sourceMappingURL=MEEServices.js.map