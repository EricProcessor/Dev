...
// 定义StorageAccountSettings每个参数的属性
//accessKey:访问密钥
//accountName:账户名称
...
export interface StorageAccountSettings {
    ...
    retryPolicy: RetryPolicyFilter.IRetryPolicy;  /*数据库引入的类型RetryPolicyFilter下面的IRetryPolicy*/
}
// 表格设置
...
EntityResolver由Azure存储检索用来转换实体。它应该是类型格式：（entityResult：Object）=>Object
*/
// 实体解析器
 ...
 */
export class AzureTable<Model> { // Slight template hack as typescript doesn't support multiple inheritances.作为类型脚本的轻微模板黑客不支持多个继承。
