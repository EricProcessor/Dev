  ...
  <view class="orderItem" v-if="options.addHide">
    <view class="topTitle">
      ...
      <view class="item">
        <text
          class="proTag"
        >{{ options.state==10?'待付款':options.state==20?'待发货':options.state==30?'待收货':options.state==99?'订单完成':'待评价' }}</text>
    ...
    <view class="productcount">
      ...
      <text>
        实付款：
        <text class="pricestyle">¥{{ options.paymentPrice }}</text>
      </text>
    </view>
    ...
    <view class="titleTag">
      ...
      <template v-if="options.state==991">
